...
exports.handler = async (event, context) => {
    ...
    const path = event.path.replace('/api/v1/chat', '');

    try {
        // Handle POST /chat endpoint (main chat completion)
        if (event.httpMethod === 'POST' && (path === '' || path === '/')) {
            const { message, mode = 'general', model, messages = [] } = JSON.parse(event.body);
            ...
            const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
                ...
                headers: {
                    ...
                    'Content-Type': 'application/json',
                    'HTTP-Referer': event.headers.referer || 'https://materioa.netlify.app',
        ...
        if (event.httpMethod === 'GET' && path === '/models') {
