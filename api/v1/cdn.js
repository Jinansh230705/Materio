...
async function batchUploadGitHubFiles(event, octokit, owner, repo, origin) {
  try {
    ...
    console.log(`Starting batch upload: ${parsedData.files.length} files for ${subject} - ${category}`);
    ...
    const commitMessage = `Batch upload: ${uploadedFiles.length} files for ${subject} - ${category}${autoPushNotify ? ' (with notification)' : ''}`;
    ...
    console.log('Batch upload completed successfully with single commit:', newCommit.sha);

    return {
      statusCode: 200,
      headers: { ...corsHeaders(origin), 'Content-Type': 'application/json' },
      body: JSON.stringify({
        message: `Successfully uploaded ${uploadedFiles.length} files in a single commit`,
