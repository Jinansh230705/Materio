...
submitButton.addEventListener('click', async () => {
    ...
    if (typeof window.loadPdfWithCache === 'function') {
        ...
        const pdfUrl = `https://cdn-materioa.netlify.app/pdfs/${semester}/${subject}/${topic}.pdf`;        document.getElementById('popupContent').innerHTML =
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (tabSwitcherCard) {
        ...
        function updateIndicatorPosition(targetTab) {
            if (activeIndicator) {
                // Simple positioning: 0% for first tab, 50% for second tab
                const leftPosition = targetTab === 'preferences' ? '4px' : 'calc(50% + 4px)';
